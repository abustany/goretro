...
const notes: Note[] = [
  ...
  {author: user1name, mood: 'positive', text: 'I got a new hat'},
...
Scenario('Test everything', async (I) => {
  ...
  // we should see all the notes, this times with the author

  for (const note of notes) {
    iSeeNote(I, note.mood, note.text, note.author);
  }
...
function saveNote(I: CodeceptJS.I, mood: Mood, text: string) {
  ...
  I.click(locate('$noteeditor-add').inside(columnTestId(mood)));
}
